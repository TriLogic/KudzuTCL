...
Template evaluation: In KudzuTCL a template is evaluated as written.  Once the template engine is invoked the engine does all the important work of tag evaluation and substitution making callbacks to stock and custom tag handlers when and where appropriate.  This means that if you adhere to the engine's callback model it's possible to move large blocks of your template without ever needing to change the code that supports it.
